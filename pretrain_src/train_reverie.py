...
def main(opts):
    ...
    train_nav_db = ReverieTextPathData(
        ...
        objv2_ft_file=data_cfg.objv2_ft_file,
        cat_file=data_cfg.cat_file,
        args=model_config, tok=tokenizer,
        max_objv2=model_config.max_objv2
    )
    val_nav_db = ReverieTextPathData(
        ...
        obj_prob_size=model_config.obj_prob_size,
        max_txt_len=opts.max_txt_len, max_objects=opts.max_objects, in_memory=True,
        objv2_ft_file=model_config.objv2_ft_file,
        cat_file=model_config.cat_file,
        args=model_config, tok=tokenizer,
        max_objv2=model_config.max_objv2
    )
...
def validate(model, val_dataloaders, setname='',max_metrix=None):
    model.eval()
    max_update_flag = False

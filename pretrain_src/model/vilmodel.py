...
class ImageEmbeddings(nn.Module):
    ...
    def forward(
        ...
        bs, view_len, obj_len = traj_obj_img_fts.shape[0], traj_obj_img_fts.shape[1], traj_obj_img_fts.shape[2]

        traj_view_img_embeds = self.img_layer_norm(self.img_linear(traj_view_img_fts))
        obj_bbox_fts = traj_obj_img_fts[:,:,:,:-1].to(torch.float32)
        obj_name_fts = traj_obj_img_fts[:,:,:,-1].int()
        obj_bbox_fts_linear = self.obj_bbox_linear(obj_bbox_fts)
        obj_name_fts_linear = self.obj_name_linear(obj_name_fts)
        obj_fuse_linear = obj_bbox_fts_linear + obj_name_fts_linear

        obj_nav_fts = self.obj_nav_type_embedding(traj_nav_types).unsqueeze(-2) # [bs,36,1,dim]
        obj_fuse_linear = obj_nav_fts + obj_fuse_linear
        obj_fts = self.obj_layer_norm(obj_fuse_linear) # [bs,36,obj_len,dim]

        obj_fts_re = rearrange(obj_fts,"b v o d -> b (v o) d")
        obj_re_masks = gen_seq_masks(traj_vp_obj_lens.reshape(-1)).reshape(-1,1,1,view_len*obj_len) # [bs*v_len*obj_len]
        obj_fts = self.obj_self_encoder(obj_fts_re,obj_re_masks)[0]
        ...
        if self.pano_encoder is not None:
            ...
            vis_obj_embeds = torch.cat((traj_self_embeds,traj_obj_embeds),-1)
            traj_embeds = self.vis_obj_linear(vis_obj_embeds)
        ...
        traj_fused_weight = self.adaptive_pano_attn(traj_ori_embeds) # [bs,view_len,1]

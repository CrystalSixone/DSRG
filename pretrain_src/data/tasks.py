...
from .common import pad_tensors, gen_seq_masks, pad_tensors_obj
...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            output['traj_objv2_fts'] = [torch.from_numpy(x) for x in inputs['traj_obj_fts']]
            output['traj_objv2_lens'] = [torch.from_numpy(x) for x in inputs['traj_obj_lens']]
        except Exception:
            output['traj_objv2_fts'] = [torch.from_numpy(x) for x in inputs['traj_objv2_fts']]
            output['traj_objv2_lens'] = [torch.from_numpy(x) for x in inputs['traj_objv2_lens']]
...
def mlm_collate(inputs):
    ...
    if 'traj_objv2_fts' in batch:
        batch['traj_objv2_lens'] = pad_tensors(sum(batch['traj_objv2_lens'], []))
        batch['traj_objv2_fts'] = pad_tensors_obj(sum(batch['traj_objv2_fts'], []))

...
from r2r.data_utils import construct_instrs, ObjectFeaatureDBv2
...
def build_dataset(args, rank=0, is_test=False):
    ...
    obj_db = ObjectFeaatureDBv2(args, args.obj_ft_file,cat_file=args.cat_file) # To load objects
    ...
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            ...
            is_test=is_test,for_debug=args.for_debug,
        ...
        aug_env = dataset_class(
            ...
            obj_db=obj_db, max_objects=args.max_objects, args=args
        )
    else:
        aug_env = None
    ...
    return train_env, val_envs, aug_env, bert_tok


def train(args, train_env, val_envs, rank=-1, bert_tok=None):
...
def main():
    ...
    train_env, val_envs, aug_env, bert_tok = build_dataset(args, rank=rank, is_test=args.test)

    if args.train == 'navigator':
        train(args, train_env, val_envs, rank=rank, bert_tok=bert_tok)

...
class ObjectFeatureDB(object):
    def __init__(self, args, obj_ft_file, obj_feat_size,cat_file=None):
        self.args = args
                    ...
                    for attr_key, attr_value in f[key].attrs.items():
                        if attr_key == 'names':
                            for i in range(len(attr_value)):
                                    _, attr_value[i] = preprocess_name(attr_value[i],self.cat_mapping,self.category_number,self.lem)
    ...
    def get_object_feature(
        ...
        if len(obj_fts) > 0:
            ...
            obj_names = np.array(obj_attrs['names'],dtype=np.int)
...
def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512, is_test=True, for_debug=False, tok=None, word_picker=None):
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits, tokenizer, is_test=is_test)):
        ...
        for j, instr in enumerate(item['instructions']):
            ...
            if tok is not None:
                new_item['instr_encoding'] = tok(instr,pad_to_max_length=True,truncation=True,max_length=max_instr_len)
            else:

...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0, tok=None):
        ...
        if self.args.use_lr_sch:
            self.vln_bert_lr_sch = get_scheduler(name=self.args.lr_sch,optimizer=self.vln_bert_optimizer,
                                                num_warmup_steps=0.1*self.args.iters,num_training_steps=self.args.iters)
            if self.default_gpu:
    ...
    def test(self, use_dropout=False, feedback='argmax', iters=None, viz=False):
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            if self.args.train_alg == 'imitation':
                ...
                self.feedback = 'expl_sample' if self.args.expl_sample else 'sample' # default:sample

...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        ...
        self.obj_db = obj_db
        self.obj_v2_db = obj_v2_db
        self.max_objv2_objects = args.max_objv2_objects
    ...
    def make_candidate(self, feature, scanId, viewpointId, viewId, objv2_bbox_fts=None, objv2_name_fts=None, objv2_lens=None):
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                if objv2_bbox_fts is not None:
                    obj_len = objv2_lens[ix]
                    obj_bbox_feat = objv2_bbox_fts[ix]
                    if objv2_name_fts is not None:
                        obj_name_feat = objv2_name_fts[ix]
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    if objv2_bbox_fts is not None:
                        adj_dict[loc.viewpointId]['objv2_bbox_feat'] = obj_bbox_feat
                        # [x,y,w,h,area,heading,elevation,cat]
                        if objv2_name_fts is not None:
                            adj_dict[loc.viewpointId]['objv2_name_feat'] = obj_name_feat
                        adj_dict[loc.viewpointId]['objv2_len'] = obj_len
                        adj_dict[loc.viewpointId]['objv2_ang_feat'] = obj_ang_pad_feature # [20,angle_feat_size]
            ...
            for c in candidate:
                ...
                if objv2_bbox_fts is not None:
                    obj_bbox_feat = objv2_bbox_fts[ix]
                    obj_len = objv2_lens[ix]
                    c_new['objv2_bbox_feat'] = obj_bbox_feat
                    if objv2_name_fts is not None:
                        c_new['objv2_name_feat'] = objv2_name_fts[ix]
                    c_new['objv2_len'] = obj_len
                    obj_ang_pad_feature = np.zeros([obj_bbox_feat.shape[0], self.angle_feat_size])
                    for obj_idx in range(obj_len):
                    ...
                    c_new['objv2_ang_feat'] = obj_ang_pad_feature
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            # object v2 for all images
            objv2_bbox_fts, objv2_name_fts, objv2_lens = None, None, None
            objv2_bbox_fts, objv2_lens = self.obj_v2_db.get_object_feature(
                state.scanId, state.location.viewpointId,
                max_objects=self.max_objv2_objects,
            )

            # Full features
            candidate = self.make_candidate(feature, state.scanId, state.location.viewpointId, state.viewIndex,
                objv2_bbox_fts, objv2_name_fts, objv2_lens
            )
            ...
            if self.obj_v2_db is not None:
                ob['objv2_pano_box_fts'] = objv2_bbox_fts 
                ob['objv2_lens'] = objv2_lens
                obj_ang_pad_feature = np.zeros([36, objv2_bbox_fts.shape[1], self.angle_feat_size])
                for viewid in range(36):
                    for obj_idx in range(objv2_lens[viewid]):
                        obj_heading = objv2_bbox_fts[viewid][obj_idx][5] + state.heading
                        obj_elevation = objv2_bbox_fts[viewid][obj_idx][6] + state.elevation
                        obj_angle_feat = angle_feature(obj_heading, obj_elevation, self.angle_feat_size)
                        obj_ang_pad_feature[viewid][obj_idx] = obj_angle_feat
                ob['objv2_ang_feat'] = obj_ang_pad_feature

...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_objv2_fts, batch_objv2_lens = [], []
        batch_objv2_nav_types = []
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                ...
                nav_types.append(1)
                objv2_nav_types.append(1)
                cand_vpids.append(cc['viewpointId'])
                used_viewidxs.add(cc['pointId'])
                view_objv2_fts.append(np.concatenate([cc['objv2_bbox_feat'][:,:2],cc['objv2_bbox_feat'][:,4:5],cc['objv2_ang_feat'],cc['objv2_bbox_feat'][:,-1].reshape(-1,1)],-1))
                view_objv2_lens.append(cc['objv2_len'])
            
            objv2_nav_types.extend([0] * (36 - len(used_viewidxs)))
            # non cand views
            view_img_fts.extend([x[:self.args.image_feat_size] for k, x \
                in enumerate(ob['feature']) if k not in used_viewidxs])
            view_ang_fts.extend([x[self.args.image_feat_size:] for k, x \
                in enumerate(ob['feature']) if k not in used_viewidxs])
            for k,x in enumerate(ob['objv2_pano_box_fts']):
                if k not in used_viewidxs:
                    tmp_obj_fts = np.concatenate([ob['objv2_pano_box_fts'][k][:,:2],ob['objv2_pano_box_fts'][k][:,4:5],
                                    ob['objv2_ang_feat'][k],ob['objv2_pano_box_fts'][k][:,-1].reshape(-1,1)
                                ],1)
                    view_objv2_fts.extend([tmp_obj_fts])
                    view_objv2_lens.append(ob['objv2_lens'][k])
            ...
            objv2_fts = np.stack(view_objv2_fts, 0)
            ...
            batch_objv2_nav_types.append(torch.LongTensor(objv2_nav_types))
            batch_cand_vpids.append(cand_vpids)
            batch_objids.append(ob['obj_ids'])
            batch_view_lens.append(len(view_img_fts))
            batch_obj_lens.append(len(ob['obj_img_fts']))
            batch_objv2_fts.append(torch.from_numpy(objv2_fts))
            batch_objv2_lens.append(view_objv2_lens)
        ...
        batch_objv2_nav_types = pad_sequence(batch_objv2_nav_types, batch_first=True, padding_value=0).cuda()
        batch_view_lens = torch.LongTensor(batch_view_lens).cuda()
        batch_obj_lens = torch.LongTensor(batch_obj_lens).cuda()

        batch_objv2_fts = pad_tensors_obj(batch_objv2_fts).float().cuda()
        batch_objv2_lens = torch.LongTensor(pad_list(batch_objv2_lens)).cuda()
        return {
            ...
            'objv2_lens': batch_objv2_lens, 'objv2_img_fts': batch_objv2_fts,
            'objv2_nav_types': batch_objv2_nav_types
        }
    ...
    def _nav_vp_variable(self, obs, gmaps, pano_embeds, cand_vpids, view_lens, obj_lens, nav_types, objv2_lens, last_embeds=None):
        batch_size = len(obs)
        ...
        objv2_lens = torch.cat([torch.zeros(batch_size,1).cuda(),objv2_lens],1)
            ...
            if ended[i]:                                            # Just ignore this index
                ...
                if ob['viewpoint'] == ob['gt_path'][-1]:
                    ...
                    for j, vpid in enumerate(vpids[i]):
                        bound = 1
                        if j > bound and ((visited_masks is None) or (not visited_masks[i][j])):
                ...
                if i_vp not in ob['gt_end_vps']:
                    ...
                    for j, obj_id in enumerate(i_objids):
                        if str(obj_id) == str(ob['gt_obj_id']):
                            targets[i] = j + view_lens[i] + 2 # 0 for CLS, 1 for MEM
                            break
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            last_embeds = nav_outs['cls_embeds'] # History unit

...
class AggregateAttention(nn.Module):
    ...
    def forward_mean(self,x,x_mask):
        '''
        :param x: [bs,src_len,dim]
        :param x_mask: [bs, src_len, 1]
        '''
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # for action and object tokens
        h_d = config.hidden_size
        self.act_embedding = nn.Embedding(config.word_max_action,config.hidden_size)
        self.obj_embedding = nn.Embedding(config.word_max_object,h_d)
...
class LanguageEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.act_obj_token_emb = nn.Embedding(1,config.hidden_size)
...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.image_dropout_prob)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_panorama_per_step(self,view_img_fts, objv2_img_fts, loc_fts, nav_types, view_lens, objv2_lens, 
            obj_img_fts=None, obj_lens=None, objv2_nav_types=None
            ):
        device = view_img_fts.device
        has_obj = obj_img_fts is not None

        img_embeds = self.img_embeddings.img_layer_norm(
            self.img_embeddings.img_linear(view_img_fts)
        ) 
        
        '''
        Fine-grained object embedding
        '''
        bs, view_len, objv2_len = objv2_img_fts.shape[0], objv2_img_fts.shape[1], objv2_img_fts.shape[2]
        objv2_bbox_fts = objv2_img_fts[:,:,:,:-1]
        objv2_name_fts = objv2_img_fts[:,:,:,-1].int()
        objv2_bbox_fts_linear = self.img_embeddings.obj_bbox_linear(objv2_bbox_fts)
        objv2_name_fts_linear = self.img_embeddings.obj_name_linear(objv2_name_fts)
        objv2_fuse_linear = objv2_bbox_fts_linear + objv2_name_fts_linear
        # add nav type to obj
        objv2_nav_fts = self.img_embeddings.obj_nav_type_embedding(objv2_nav_types).unsqueeze(-2) 
        objv2_fuse_linear = objv2_nav_fts + objv2_fuse_linear
        objv2_fts = self.img_embeddings.obj_layer_norm(objv2_fuse_linear) # [bs,36,obj_len,dim]
        if self.img_embeddings.obj_self_encoder is not None:
            obj_fts_re = rearrange(objv2_fts,"b v o d -> b (v o) d")
            obj_re_masks = gen_seq_masks(objv2_lens.reshape(-1)).reshape(-1,1,1,view_len*objv2_len) # [bs*v_len*obj_len]
            objv2_fts = self.img_embeddings.obj_self_encoder(obj_fts_re,obj_re_masks)[0]
            objv2_fts = rearrange(objv2_fts,"b (v o) d -> b v o d",v=view_len)
            traj_obj_embeds = objv2_fts.mean(dim=2) # [bs,v,d]

        vis_obj_embeds = torch.cat((img_embeds,traj_obj_embeds),-1) # [bs,view_len,dim+obj_dim]
        view_img_embeds = self.img_embeddings.vis_obj_linear(vis_obj_embeds) # [bs,view_len,dim]

        if has_obj:
            obj_img_embeds = self.img_embeddings.obj_reverie_layer_norm(
                self.img_embeddings.obj_reverie_linear(obj_img_fts)
            )
            ...
            traj_img_embeds = pad_tensors_wgrad(img_embeds)
            traj_vp_lens = view_lens + obj_lens
        else:
        ...
        return traj_self_embeds, pano_masks, traj_fused_embeds
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            pano_embeds, pano_masks, pano_fused_embeds = self.forward_panorama_per_step(
                batch['view_img_fts'], batch['objv2_img_fts'], batch['loc_fts'],
                batch['nav_types'], batch['view_lens'], batch['objv2_lens'],
                batch['obj_img_fts'], batch['obj_lens'], batch['objv2_nav_types']
            )
